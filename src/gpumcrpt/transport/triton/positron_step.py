...
@triton.jit
def positron_condensed_step_kernel(
    ...
    """
    Positron condensed-history step (mirrors electron kernel):
      - continuous loss dE = S_restricted * ds (energy conserving via edep atomic)
      - step length ds limited by voxel and range
      - optional brems/delta event flags from per-cm rates
      - if E drops below cutoff: mark stop=1 (annihilation at rest will be handled outside)
        and do NOT keep it in alive queue.
    """
    pid = tl.program_id(0)
    ...
    R = tl.load(range_csda_ptr + off, mask=inside, other=1e-3)               # cm
    ...
    # move (no multiple scattering in MVP)
    z2 = z + ds * uz
    y2 = y + ds * uy
    x2 = x + ds * ux

    # determine stop at rest
    stop = inside & (E2 < e_cut_MeV)
    alive = inside & (E2 >= e_cut_MeV)

    # hard-event flags only if alive
    ...
    tl.store(out_emit_brem_ptr + offs, emit_b.to(tl.int8), mask=True)
    tl.store(out_emit_delta_ptr + offs, emit_d.to(tl.int8), mask=True)
