...
@triton.jit
def electron_condensed_step_record_kernel(
    ...
    """
    Record-mode condensed-history step for electrons:
      - DOES NOT do tl.atomic_add into edep
      - Writes (lin, dE*w) so host/orchestrator can sort+reduce and then do a small number of adds.

    Note: hard-event flags (brems/delta) are still emitted here; secondaries handled outside.
    """
    pid = tl.program_id(0)
