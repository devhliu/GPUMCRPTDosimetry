...
def photon_photoelectric_with_vacancy_kernel(
    ...
    m = offs < N
    E = tl.load(E_ptr + offs, mask=m, other=0.0).to(tl.float32)
    w = tl.load(w_ptr + offs, mask=m, other=0.0).to(tl.float32)
    rng = tl.load(rng_ptr + offs, mask=m, other=1234567).to(tl.int32)

    # position
    z = tl.load(pos_ptr + offs * 3 + 0, mask=m, other=0.0)
    y = tl.load(pos_ptr + offs * 3 + 1, mask=m, other=0.0)
    x = tl.load(pos_ptr + offs * 3 + 2, mask=m, other=0.0)
    ...
    base = mat * S
    cdf0 = tl.load(shell_cdf_ptr + base + 0, mask=m, other=1.0)
    ...
    for s in range(1, 8):  # supports S<=8
        c = tl.load(shell_cdf_ptr + base + s, mask=m, other=1.0)
        shell += (u > c).to(tl.int32)

    shell = tl.minimum(shell, S - 1)

    E_bind = tl.load(E_bind_ptr + base + shell, mask=m, other=0.0).to(tl.float32)
    Ee = tl.maximum(E - E_bind, 0.0)

    # create photoelectron (direction: reuse photon direction; you can isotropize later if desired)
    ux = tl.load(dir_ptr + offs * 3 + 2, mask=m, other=1.0)
    uy = tl.load(dir_ptr + offs * 3 + 1, mask=m, other=0.0)
    uz = tl.load(dir_ptr + offs * 3 + 0, mask=m, other=0.0)

    tl.store(out_e_pos_ptr + offs * 3 + 0, z, mask=m)
    tl.store(out_e_pos_ptr + offs * 3 + 1, y, mask=m)
    tl.store(out_e_pos_ptr + offs * 3 + 2, x, mask=m)
    tl.store(out_e_dir_ptr + offs * 3 + 0, uz, mask=m)
    tl.store(out_e_dir_ptr + offs * 3 + 1, uy, mask=m)
    tl.store(out_e_dir_ptr + offs * 3 + 2, ux, mask=m)
    tl.store(out_e_E_ptr + offs, Ee, mask=m)
    tl.store(out_e_w_ptr + offs, w, mask=m)
    tl.store(out_e_rng_ptr + offs, rng, mask=m)

    # vacancy output
    has_vac = m & inside & (E_bind > 0.0)
    tl.store(out_vac_pos_ptr + offs * 3 + 0, z, mask=m)
    tl.store(out_vac_pos_ptr + offs * 3 + 1, y, mask=m)
    tl.store(out_vac_pos_ptr + offs * 3 + 2, x, mask=m)
    tl.store(out_vac_mat_ptr + offs, mat.to(tl.int32), mask=m)
    tl.store(out_vac_shell_ptr + offs, shell.to(tl.int8), mask=m)
    tl.store(out_vac_w_ptr + offs, w, mask=m)
    tl.store(out_has_vac_ptr + offs, has_vac.to(tl.int8), mask=m)
