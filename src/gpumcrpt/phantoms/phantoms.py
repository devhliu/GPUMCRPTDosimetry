...
class PhantomVolumes:
    ct_hu: nib.Nifti1Image
    activity_bqs: nib.Nifti1Image
...
class NemaIecBodyPhantomVolumes(PhantomVolumes):
    ...
    sphere_label: nib.Nifti1Image | None = None
...
def make_water_slab_with_bone_cylinder(
    *,
    ...
    Output units
    - ct_hu.data: HU
    - activity_bqs.data: Bq*s per voxel (Time-Integrated Activity for source sampling)
    ...
    ct_img = nib.Nifti1Image(ct, affine=affine)
    act_img = nib.Nifti1Image(act, affine=affine)

    return PhantomVolumes(ct_hu=ct_img, activity_bqs=act_img)
