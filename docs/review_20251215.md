...
The physics design is solid: you’ve clearly separated Domain A (decay/source term) from Domain B (transport) and you’ve written down the key “no double counting / energy conservation” rules (neutrinos ignored, annihilation handled in transport, cutoff deposits remaining 
𝐸
𝑘
E 
k
...
There are several “wiring completeness” problems that will block users from running the package end-to-end:
...
configs/example_simulation.yaml is only 13 lines and doesn’t contain required config blocks that pipeline.py expects (io, materials, physics_tables, decaydb, nuclide, cutoffs, etc.).
...
If you want to keep “≤1 line per category per history” for performance, the correct way is usually weighted splitting: sample at most one emission, but scale particle weight by expected multiplicity/yield so the estimator stays unbiased (and don’t sum energies).
Atomic relaxation “effective Z per material” is a good MVP, but it’s a known approximation:
Next step is to move from an “effective Z” to elemental fractions per material class (even a small set: H/C/N/O/Ca/P for tissue/bone) so shell probabilities and binding energies are more faithful.
...
Interaction-specific checks: Compton doesn’t locally deposit recoil energy if recoil electron is created; pair doesn’t locally deposit 1.022 MeV; brems/δ subtract emitted energy from parent.
GPU performance/architecture suggestions

Pick one “current engine” and integrate phases into a single production path:
...
Recommendation: choose Phase 11 “one sync per step” (it’s a pragmatic compromise) and make it the default engine, then keep the fully GPU-resident variant behind a flag.
...
Your plan is correct: separate photon/electron graphs, capture micro-steps per bucket size. The next development step is to make the step function accept a fixed “bucketed” active count and run exactly k micro-iterations per capture.
Software engineering / usability

Add pytest to requirements.txt (tests exist, but pytest isn’t installed right now).
Provide one runnable “toy” entrypoint:
A minimal script that builds toy H5 tables, synthesizes a small CT/activity volume, runs TransportEngine, writes dose NIfTI. This becomes your “golden” smoke test and onboarding path.
...
Milestone 2: “Physics baseline validation”
Photon-only water slab vs expected attenuation + energy deposition; then add Compton+PE; then add positrons+annihilation; then relaxation.
Milestone 3: “Performance stabilization”
...
HU→materials with elemental compositions, better relaxation mapping, uncertainty outputs, and comparisons against Geant4/GATE for a reference phantom.
If you want, I can turn the “Runnable MVP” into concrete patches (fix return tables, save unc, reconcile imports, and expand the example config) so the package can execute an end-to-end toy run.
...
Added generic “run pipeline on CT/activity NIfTI” script (for ICRP110-like inputs too): run_dosimetry_nifti.py.
...
The main physics-correctness issue relative to your PDF/CDF guidance was in source sampling: it was summing energies from multiple emission categories into a single photon/electron per history. That’s unphysical because interaction/transport depends nonlinearly on particle energy.
...
Avoid the old “energy summation across categories” shortcut.
...
Beta spectra are sampled via inverse-CDF from the tabulated PDF; discrete lines use yield-weighted sampling (discrete PDF → CDF sampling).
...
If you want, I can do the next accuracy pass you asked for on “transport physics correctness” (beyond the source term): review where PDFs/CDFs are used in photon/electron interactions and relaxation, and identify/patch any places that are still using placeholders or non-invertible sampling.
------------------------------------