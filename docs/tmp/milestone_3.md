...
Goal
- Extend Milestone 2 (photon-electron-local) to include condensed-history transport for electrons and positrons, plus positron annihilation-at-rest that emits two 511 keV photons.
...
How to run
- Config switch (pipeline/engine selector): set `monte_carlo.triton.engine: photon_electron_condensed`.
...
- Toy runner:
	- `PYTHONPATH=src python scripts/run_toy_em_condensed.py --tables toy_physics.h5 --out toy_edep_photon_electron_condensed.pt`

Where the implementation lives
- Engine: `src/gpumcrpt/transport/engine_gpu_triton_photon_electron_condensed.py`
...
Created engine_gpu_triton_photon_electron_condensed.py
...
I’ve implemented and wired the Milestone-3 backend (photon_electron_condensed) into the existing TransportEngine. Next up: add a CUDA-smoke test + a toy runner, then run the full pytest suite again.
...
New engine: engine_gpu_triton_photon_electron_condensed.py
...
Engine selection wired via config: set monte_carlo.triton.engine: photon_electron_condensed in engine.py and documented in example_simulation.yaml
...
Next step recommendation: add a tiny “engine selector sanity” test that checks the allowed Triton engine strings (mvp, photon_electron_local, photon_electron_condensed) are accepted and anything else fails fast (no GPU execution; just config/validation logic).
--------------------------------------------------
