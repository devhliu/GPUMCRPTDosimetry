...
## Root Cause Analysis
### 1. GPU Performance Issues Critical Memory Allocation Overhead in photon_electron_condensed Mode
In engine_gpu_triton_photon_electron_condensed.py , output buffers are allocated inside the main loop for every iteration:
...
Contrast with photon_electron_local mode : In engine_gpu_triton_photon_electron_local.py , buffers are allocated once outside the loop .
...
```
torch.nonzero() and torch.index_select() cause CPU-GPU synchronization, blocking GPU execution. This happens multiple times per iteration in photon_electron_condensed mode for photoelectric, Compton, and pair production events.
 Excessive Secondary Particle Handling
The photon_electron_condensed mode has deep secondary particle handling with multiple select_indices_with_budget calls and recursive secondary depth checks. With default max_secondaries_per_primary=1_000_000_000 , this can spawn massive numbers of secondary particles, overwhelming the system.
...
1. Move buffer allocations outside the loop in photon_electron_condensed mode:
...
5. Clarify photoelectric handling :
   ...
   - Or fully deposit energy locally (consistent with photon_electron_local)
...
Review GPU performance issues in photon_electron_local and photon_electron_condensed modes
